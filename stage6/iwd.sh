...
if [ $? = 0 ]; then
    ...
    pushd $LFS/sources/$(getConf LFS_VERSION)
        ...
        while [ -z $PKG_PATH ]
        do
            wget https://www.kernel.org/pub/linux/network/wireless/iwd-2.2.tar.xz
            PKG_PATH=$(find . -maxdepth 1 -type f -name "$PKG_NAME-*.*")
            sleep 1
        done
        ...
        while [ -z $PKG_PATH ]
        do
            wget https://www.kernel.org/pub/linux/libs/ell/ell-0.56.tar.xz
            PKG_PATH=$(find . -maxdepth 1 -type f -name "$PKG_NAME-*.*")
            sleep 1
        done
    popd
...
pushd /sources/_LFS_VERSION
    ...
    if [ ! -f $PKG_PATH/_BUILD_DONE ]; then
        pushd $PKG_PATH
            ...
            [ $? = 0 ] && make
            [ $? = 0 ] && (make -k check || /bin/true) && read -p "$PKG_NAME CHECK DONE..."
            [ $? = 0 ] && make install
            if [ $? = 0 ]; then
                read -p "$PKG_NAME ALL DONE..."
                touch _BUILD_DONE
